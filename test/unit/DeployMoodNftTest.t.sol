...
contract DeployMoodNftTest is Test {
    ...
    function testGetSvgUriTakesSvgFilePathAndReturnsUri() external view {
        string memory happySvgPath = "img/dynamic/happy.svg";
        string memory sadSvgPath = "img/dynamic/sad.svg";

        assert(
            getBytes(deployMoodNft.getSvgUri({filePath: happySvgPath})) ==
                getBytes(HAPPY_SVG_IMAGE_URI)
        );

        assert(
            getBytes(deployMoodNft.getSvgUri({filePath: sadSvgPath})) ==
                getBytes(SAD_SVG_IMAGE_URI)
        );
    }
